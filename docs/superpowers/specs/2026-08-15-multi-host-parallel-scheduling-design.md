...
worker 循环中已有的 `isUp()` 指数退避保持不变，一台离线不影响其他 worker 继续认领。**但正在该主机上执行的 job 不能再无限等待**——见下节。
...
| 要迁的东西 | 实际情况 |
|---|---|
| job 行状态 | `resetJobToPending` 已清 `comfy_prompt_id` 与 `started_at`；`host_id` 在重新认领时覆盖 |
| 输入图片 | `gpuUploads` 是 per-worker 的，新主机认领后自动重新上传本地 uploads 的图——现有 `execute()` 逻辑不变 |
| 半成品输出 | ComfyUI 不暴露中间态，放弃即丢弃，无可迁移之物 |
| **引用 GPU 侧已有文件的任务** | **搬不动**，但已被 `pinned_host_id` 挡在池外，不会走到迁移 |

因此迁移 = **把 job 放回 pending 池**。`pinned_host_id` 在此处的作用比防止误派更根本：它是「迁移可以简化成一次 reset」的前提。

**附带修正**：`resetJobToPending` 目前不清 `host_id`，导致回池的 pending 任务在 UI「主机」列仍显示旧主机。回池时一并置空——pending 任务不该声称属于某台主机。

### 什么时候放回池

| 场景 | 处理 |
|---|---|
| 主动停用（interrupt 模式） | abandon → 回池，由其他主机接手 |
| **主机中途离线超过 2 分钟** | **新增**：放弃当前 job → 回池 |
| 任务执行失败 | **不迁移**，直接 failed（理由见下） |
| 熔断停用 | 该主机没有在跑的任务；其 pending 任务本就不绑主机，自然被其他主机认领 |

#### 离线迁移（新增）

单主机时代「离线就无限等重连」是正确的（主机回来接着跑，不浪费已算的部分）；并行后它会制造**僵尸任务**——其他主机照常干活，这一个永远 running，batch 永远完不成。

实现复用现有机制：`waitForHistory` 的 catch 分支已有 `UNREACHABLE_ABANDON_POLLS = 3` 判定，目前只在 `!this.running`（pause 等待中）生效。改为**运行中的 worker 也适用**，判定口径从「轮数」改为**连续不可达时长 ≥ 2 分钟**（轮询有指数退避，轮数不等价于时长）。

超时后抛 `AbandonError` → 现有链路把 job 重置回 pending → 其他主机接手。**注意**：该 job 的失败不计入熔断计数（`AbandonError` 本就不计），主机不可达不是主机「坏」。

2 分钟阈值为常量：ComfyUI 重启通常 30 秒内完成，2 分钟能遮住绝大多数抖动。

#### 为什么失败不自动重派

不做「换一台主机重试一次」，也不做主机 × 模板的兼容性建模（比对模板所需 checkpoint/LoRA/自定义节点与各主机 `/object_info`）。**熔断已经是这个问题的解**：

主机 A 缺模型 → 任务在 A 失败 → 连续 3 次 → A 被移出调度池 → 之后的任务（含用户手动 retry 的）自然只落到有能力的主机。

这是「运行时发现」而非「预先建模」。代价是每台不合格主机浪费 3 个任务；换来零配置、零维护，且对「模型被删/新装、ComfyUI 升级换节点名」这类变化**自动适应**——预先建模反而要处理缓存失效，并可能因对比逻辑跟不上 ComfyUI 变化而误判。

自动重派的代价则是：参数写错这类真正有问题的任务会烧掉两台主机的时间，且需要在 job 上记录已试过哪些主机。
...
12. **离线迁移**：主机不可达达 2 分钟 → job 回池且 `host_id` 置空 → 被其他主机认领；不可达期间未达阈值则继续等待；该放弃**不计入熔断计数**（用假时钟推进，不真等 2 分钟）
...
- 不做主机 × 模板的兼容性建模；坏主机靠熔断在运行时被移出调度池
- 任务失败不自动换主机重试；离线迁移是唯一的自动迁移路径
