...
- 生成图为底图：`max-h-[70vh] max-w-full`（保比缩放，容器收缩包裹实际绘制区，无留白）。
- 原图绝对定位叠放，`object-fit: fill` 拉伸到与生成图完全相同的矩形（ComfyUI 尺寸取整 /8 造成的微小比例差由拉伸吸收，边缘逐边对齐；验收修订：原「各自 object-contain 留白」方案会产生数像素至十几像素错位），`clip-path: inset(0 X% 0 0)` 裁掉右侧 → 分割线左边原图、右边生成图。
...
- 比例差异大的极端场景原图会被明显拉伸变形，接受（编辑类工作流输入输出比例天然接近）。
...
- 拖动滑杆 / 点击锁定交互（选定悬停跟随）。
- 缩放同步、放大镜、按 naturalSize 量取矩形的 JS 对齐（CSS 拉伸对齐已足够）。
- 画廊缩略图上的对比入口。
